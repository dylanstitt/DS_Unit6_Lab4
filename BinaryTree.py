...
class BinaryTree:
    ...
    def breadth_traversal(self):
        ...
        for i in range(self.__size):
            ...
            if node._BinaryNode__left is not None:
                queue.enqueue(node._BinaryNode__left)

            if node._BinaryNode__right is not None:
                queue.enqueue(node._BinaryNode__right)

        return result

    def get_height(self, node=None, depth=0):
        """Get the height of the tree"""
        if node is None:
            node = self.__root

        if self.is_leaf(self.__make_position(node)):
            lDepth = rDepth = 0

            if node._BinaryNode__parent._BinaryNode__left is node:
                lDepth = self.get_depth(self.__make_position(node))
            else:
                rDepth = self.get_depth(self.__make_position(node))
            ...
            if node._BinaryNode__left is not None:
                left = self.get_height(node._BinaryNode__left)
                if left > depth:
                    depth = left

            if node._BinaryNode__right is not None:
                right = self.get_height(node._BinaryNode__right)
                if right > depth:
                    depth = right
